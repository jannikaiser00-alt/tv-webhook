...
// ===================== EXCHANGE DATA (mit Cache & Rate-Limit-Log) =====================
async function fetchCandles(symbol = "SOLUSDT", interval = "15m", limit = 400) {
  const key = `${symbol}_${interval}`;
  const cached = candlesCache.get(key);
  if (cached && Date.now() - cached.ts < 15000) return cached.data; // 15s Cache-TTL
  ...
  const parsed = { highs, lows, closes, lastClose: closes[closes.length - 1] };
  candlesCache.set(key, { ts: Date.now(), data: parsed });
  return parsed;
}

async function fetchBookTicker(symbol = "SOLUSDT") {
  const cached = bookCache.get(symbol);
  if (cached && Date.now() - cached.ts < 5000) return cached.data; // 5s Cache-TTL
  ...
  const data = response.data;
  const parsed = { bid: parseFloat(data.bidPrice), ask: parseFloat(data.askPrice) };
  bookCache.set(symbol, { ts: Date.now(), data: parsed });
  return parsed;
}

function logRateLimitHeaders(headers) {
  const used = headers["x-mbx-used-weight-1m"] || headers["x-mbx-used-weight"];
  const limit = headers["x-mbx-order-count-1m"] || "?";
  if (used) {
